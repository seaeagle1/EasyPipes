...
namespace EPUnitTests
{
    public class IPC
    {
        ...
        public void EncryptedTcpTest()
        {
            ...
            Client client = new TcpClient(new IPEndPoint(IPAddress.Parse("127.0.0.1"), 8055), enc);
            client.Connect();

            IService service = client.GetServiceProxy<IService>();
            int result1 = service.Sum(9, 11);
            service.EnableEncryption();
            int result2 = service.Sum(2, 4);
